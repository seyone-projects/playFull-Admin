...
import { GetMonthlySummaryReport } from '../../service/BatchStudentPaymentService';

export default function ReportMonthlyPayments() {
  ...
  const fetchMonthlyReport = async () => {
    ...
    try {
      ...
      console.log("Monthly Payment Report:", response);

      setPayments(response.summary || []);
    } catch (error) {
      setAppError(true);
  ...
  return (
    <>
      <Helmet>
        <title>Payment Monthly Reports | {config.appName}</title>
      </Helmet>
      <div className='container'>
        <div className='page'>
          <div className='page-heading'>
            <h1>Payment Monthly Reports</h1>
            <span>
              <Link to="/"> Dashboard </Link> / Payment Monthly Reports
            </span>
          </div>
                            ...
                            {payments.map(batch => (
                              <React.Fragment key={batch.id}>
                                ...
                                <tr className="table-primary">
                                  <td colSpan="6" className="fw-bold text-uppercase">
                                    {batch.name}
                                  </td>
                                </tr>
                                ...
                                {batch.users && batch.users.length > 0 ? (
                                  ...
                                  <tr>
                                    <td colSpan="6" className="text-center text-warning fw-bold">
