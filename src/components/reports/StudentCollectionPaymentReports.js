...
import { GetCollectionReports } from '../../service/BatchStudentPaymentService';
...
export default function UserList() {
  ...
  const fetchPaymentList = async (page = 1) => {
    ...
    try {
      setIsLoading(true);
      setHasSearched(true);
      var response = await GetCollectionReports(fromDate, toDate, batchId, page, itemsPerPage);
  ...
  return (
    <>
      <Helmet>
        <title>Collection Reports | {config.appName} </title>
      </Helmet>
      <div className='container'>
        <div className='page'>
          <div className='page-heading'>
            ...
            <span>
              <Link to="/"> Dashboard </Link> / Collection Reports
            </span>
          </div>
